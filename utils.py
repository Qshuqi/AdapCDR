...
#             log2_iplus1 = np.log2(1+np.arange(1,top_k+1))
            ...
            recall = np.sum(y_u[pred_top_k]) / max(1, min(np.sum(y_u), top_k))# / log2_iplus1
...
def expected_calibration_error(y_true, y_prob, n_bins=15, boundaries=None, return_boundaries=False):
    ...
    if boundaries is None:
        ...
        candidate_split_indices = sorted(list(set(candidate_split_indices))) # a sorted list of unique indices

        # Helper function to calculate ECE for a slice of data
        def _calculate_ece_for_slice(probs, labels):
            if len(probs) == 0:
        ...
        for _ in range(n_bins - 1):
            ...
            for i in range(len(splits) - 1):
                ...
                original_ece_contrib = _calculate_ece_for_slice(sorted_probs[start_idx:end_idx], sorted_labels[start_idx:end_idx])

                # Find candidate splits within the current bin
                possible_splits = [idx for idx in candidate_split_indices if start_idx < idx < end_idx]

                for split_idx in possible_splits:
                    ece1 = _calculate_ece_for_slice(sorted_probs[start_idx:split_idx], sorted_labels[start_idx:split_idx])
                    ece2 = _calculate_ece_for_slice(sorted_probs[split_idx:end_idx], sorted_labels[split_idx:end_idx])
